...
def velocity_verlet(
    ...
    """
    # I've gone ahead and split the initial state into position and velocity
    q = x_0[: len(x_0) // 2].copy()
    v = x_0[len(x_0) // 2 :].copy()
    N = len(x_0) // 2
...
def forward_euler_integrate(
    ...
    """
    Performs an update of Euler integration for n steps.
    ...
    dt : float
        The time step for the integration.
    deriv_function : Callable
        A function that calculates the time-derivative of the system given
        the current state.


    -------
    np.ndarray
        The resulting trajectory of the system after n_steps.  Should be of
        size (n_steps+1) x M, where X is the shape of x_0.
    """
    x = x_0.copy()
    trajectory = np.zeros((n_steps + 1, len(x_0)))
    trajectory[0] = x

    for step in range(1, n_steps + 1):
        dxdt = deriv_function(x)
        x = x + dt * dxdt
        trajectory[step] = x
...
def backward_euler_integrate(
    ...
    dt : float
        The time step for the integration.
    deriv_function : Callable
        A function that calculates the time-derivative of the system given the current state.

    Returns
    -------
    np.ndarray
        The resulting trajectory of the system after n_steps.
    """
    x = x_0.copy()
    trajectory = np.zeros((n_steps + 1, len(x_0)))
    trajectory[0] = x
    ...
    for step in range(1, n_steps + 1):
        def func(x_next):
            return x_next - x - dt * deriv_function(x_next)

        x = fsolve(func, x)  # Use current x as initial guess
        trajectory[step] = x
